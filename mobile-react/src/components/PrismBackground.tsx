    ...
    return `
      ...
      <html>
        ...
        <body>
          ...
          <script type="module">
            import { Renderer, Triangle, Program, Mesh } from 'https://cdn.skypack.dev/ogl';

            // Debug: Change background to indicate script is running if we fail to render
            ...
            // Placeholder pointer interaction for mobile (using touch?)
            // For now we can just simulate or use touch events if needed.
            // The React Native WebView will handle touch events if we want to pass them,
            // but for a background, we might not need interactive hover.
            // The user's code has "hover" animation type.
            
            const pointer = { x: 0, y: 0, inside: false };
            
            if (props.animationType === 'hover') {
                // We could listen to touchmove here
                window.addEventListener('touchmove', (e) => {
                    ...
                    const cx = ww * 0.5;
                    const cy = wh * 0.5;
                    const nx = (touch.clientX - cx) / (ww * 0.5);
                    const ny = (touch.clientY - cy) / (wh * 0.5);
      ...
      <WebView
        ...
        onMessage={(event) => {
          try {
            ...
            if (data.type === 'error') {
              console.error('PrismBackground WebView Error:', data.message, data.info || data.stack);
            }
          } catch (e) {
            // ignore
          }
